...
export const getLikes = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query("select * from likes where id_tutorial=? and id_user=?", [id, id_user])
...
export const addLikes = async(id_tutorial, id_user) => {
  try {
    ...
    const[rows] = await pool.query("insert into likes (id_tutorial, id_user) values (?,?)", [id_tutorial, id_user])
...
export const deleteLikes = async(id_tutorial, id_user) => {
  try {
    ...
    const[rows] = await pool.query("delete from likes where id_user=? and id_tutorial=?", [id_user, id_tutorial])
