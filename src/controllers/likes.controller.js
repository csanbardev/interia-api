...
export const getLikes = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query("select * from t_likes where lks_id=? and lks_usr_id=?", [id, id_user])
...
export const deleteLikes = async(id_tutorial, id_user) => {
  try {
    ...
    const[rows] = await pool.query("delete from t_likes where lks_usr_id=? and lks_tut_id_tutorial=?", [id_user, id_tutorial])
